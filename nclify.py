...
def rename_signal(old_signal, kind):
...
for line in fi:
    if line.startswith(".inputs ") or line.startswith(".outputs "):
        ...
        new_names = []
        for name in names:
            new_names.append(rename_signal(name, "t"))
            new_names.append(rename_signal(name, "f"))
        ...
        for arg in a[2:]:
            parts = arg.split("=")
            assert len(parts) == 2
            new_args.append(parts[0] + "_t=" + rename_signal(parts[1], "t"))
            new_args.append(parts[0] + "_f=" + rename_signal(parts[1], "f"))
